      ...
      <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="p-4 border-b border-gray-100 space-y-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                />
              </div>
        ...
        <div className={`lg:col-span-2 ${!selectedConversation ? 'hidden lg:block' : ''}`}>
          {!selectedConversation ? (
            ...
            <div className="bg-white rounded-xl border border-gray-100 h-[600px] flex flex-col">
              {/* Chat Header */}
              <div className="p-3 sm:p-4 border-b border-gray-100 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setSelectedConversation(null)}
                    className="lg:hidden p-2 hover:bg-gray-100 rounded-lg text-gray-900"
                  >
                  ...
                  {selectedConversation.user?.image ? (
                    <img
                      ...
                      className="w-9 h-9 sm:w-11 sm:h-11 rounded-full object-cover border-2 border-amber-200"
                      referrerPolicy="no-referrer"
                    />
                  ) : (
                    <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center text-white font-semibold text-sm sm:text-lg">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs sm:text-sm text-gray-500 truncate">{selectedConversation.subject}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  >
                    {selectedConversation.status === "open" ? "Marchează ca închis" : "Redeschide"}
                  </button>
                  <button
              ...
              {/* Messages */}
              <div className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-2 sm:space-y-3 bg-gray-50 relative">
                {loadingMessages ? (
                      ...
                      <div className={`flex items-end gap-2 max-w-[75%] ${msg.isFromAdmin ? "flex-row-reverse" : ""}`}>
                        {!msg.isFromAdmin && (
                          selectedConversation.user?.image ? (
                            ...
                            <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center text-white text-[10px] sm:text-xs font-bold flex-shrink-0">
                              {selectedConversation.user?.name?.[0]?.toUpperCase() || "?"}
                            </div>
                          )
                        )}
                        {msg.isFromAdmin && (
                          <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-gradient-to-br from-amber-400 via-orange-500 to-pink-500 p-[2px] flex-shrink-0">
                        ...
                        <div
                          className={`px-3 sm:px-4 py-2 sm:py-2.5 relative group ${
                            msg.isFromAdmin
                        ...
                        >
                            ...
                            <div className={`flex items-center gap-1 mt-1 ${msg.isFromAdmin ? "justify-end" : ""}`}>
                              {msg.editedAt && (
                                <span className={`text-[10px] ${msg.isFromAdmin ? "text-white/60" : "text-gray-400"}`}>
                                  (editat)
                                </span>
                              )}
                              <span className={`text-[10px] ${msg.isFromAdmin ? "text-white/70" : "text-gray-400"}`}>
              ...
              {/* Input */}
              {selectedConversation.status === "open" && (
                <form onSubmit={editingMessage ? (e) => { e.preventDefault(); saveEdit(); } : sendMessage} className="p-3 sm:p-4 border-t border-gray-100">
                  {editingMessage && (
                    <div className="mb-2 px-3 py-2 bg-amber-50 rounded-lg text-sm text-amber-800 flex items-center justify-between">
                  ...
                  <div className="flex gap-2 sm:gap-3">
                    <input
                      ...
                      placeholder={editingMessage ? "Editează mesajul..." : "Scrie un răspuns..."}
                      className="flex-1 px-3 sm:px-4 py-2 sm:py-3 border border-gray-200 rounded-full text-gray-900 focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm"
                    ...
                    {editingMessage && (
                      <button
                        type="button"
                        onClick={cancelEdit}
                        className="p-2 sm:p-3 bg-gray-500 text-white rounded-full hover:bg-gray-600 transition-colors flex-shrink-0"
                      >
                    ...
                    <button
                      ...
                      className="p-2 sm:p-3 bg-amber-500 text-white rounded-full hover:bg-amber-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
                    >
                      {sending ? (
